...
public class GuidedPhraseLiveController {
    ...
    @PostMapping("/live-token") public synchronized ResponseEntity<?> token(@RequestBody Map<String,String> request){
        ...
        try {var access=live.createGuidedPhraseAccess(instruction(scenario));GuidedPhraseDailyPractice row=dailyPractices.findByEmailIgnoreCaseAndPracticeDate(email,today()).orElseGet(GuidedPhraseDailyPractice::new);row.setEmail(email);row.setPracticeDate(today());row.setSessionsStarted(used+1);row.setUpdatedAt(Instant.now());dailyPractices.save(row);return ResponseEntity.ok(Map.of("token",access.token(),"model",access.model(),"websocketUrl",access.websocketUrl(),"voice",access.voice(),"systemInstruction",access.systemInstruction(),"practicesRemaining",Math.max(0,DAILY_PRACTICE_LIMIT-used-1)));}catch(IllegalStateException e){return ResponseEntity.status(503).body(Map.of("message",e.getMessage()));}catch(Exception e){return ResponseEntity.status(502).body(Map.of("message","Sumi could not start the conversation. Please try again."));}
    }
    ...
    private int practicesUsed(String email){return dailyPractices.findByEmailIgnoreCaseAndPracticeDate(email,today()).map(GuidedPhraseDailyPractice::getSessionsStarted).orElse(0);}
